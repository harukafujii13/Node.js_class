...
app.post("/books/create", (req, res) => {
  ...
  const sqlInsertBook =
    "INSERT INTO Books (Title, Author, Comments) VALUES ($title, $author, $comments)";
  db.run(
    sqlInsertBook,
    {
      $title: title,
...
//memo1
//The seedDB() function is created to handle the table creation and data seeding process.
//It drops the existing "Books" table if it exists and creates a new "Books" table with the specified schema.
//It then inserts two books into the table.

//The code block that checks for the existence of the "Books" table is moved inside db.serialize().
//It queries the sqlite_master table to check if the "Books" table exists. If it doesn't exist,
//the seedDB() function is called to create the table and insert initial data.

//memo2
//db.serialize(() => { ... }) ensures that the code executes in a serialized manner within the database connection.
